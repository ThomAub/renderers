...
use crate::types::{MultiModalData, ParsedResponse, RenderError, RenderedTokens};
...
pub trait Renderer: Send + Sync + std::fmt::Debug {
    ...
    fn render_ids(
        ...
        Ok(self.render(messages, tools, add_generation_prompt)?.token_ids)
    }
...
pub trait MultimodalRenderer: Renderer {
    /// Map of placeholder token id → modality marker (1 = image, 2 = video).
    fn mm_token_type_id_map(&self) -> &[(u32, u8)];

    /// Multimodal-aware bridge. The trailing argument carries the prior
    /// turn's sidecar so placeholders survive across turns.
    fn bridge_to_next_turn_mm(
        &self,
        previous_prompt_ids: &[u32],
        previous_completion_ids: &[u32],
        new_messages: &[Message],
