...
fn bench_bridge(c: &mut Criterion) {
    ...
    let renderer = Qwen3Renderer::new(tok).expect("build Qwen3 renderer");
    let messages = typical_conversation();
    let rendered = renderer.render(&messages, None, true).expect("render");
    let prev_prompt_ids = rendered.token_ids.clone();
...
criterion_group!(benches, bench_render_ids, bench_parse_response, bench_bridge);
